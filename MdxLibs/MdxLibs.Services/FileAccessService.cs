...
public class FileAccessService {
	...
	public virtual FileInfo[] GetFiles ( DirectoryInfo dir ) => dir.GetFiles ();
	...
	public enum SearchOptions {
		...
		AllExisting = SubDirectories | ProjectFolder | SolutionFolder,
		All = 0xFFFF,
	}
	...
	public virtual string GetAssetPath ( string basePath, string filename, SearchOptions searchOptions ) {
		...
		bool CheckSubdirectories ( string path, out string potentialPath ) {
			if ( searchOptions.HasFlag ( SearchOptions.SubDirectories ) && (!allowMissingDir || Directory.Exists ( basePath )) ) {
				List<DirectoryInfo> searchedDirs = [];
				if ( GetFiles ( new (basePath) ).Any( f=>f.Extension == ".sln") ) {
					// Projects can be grouped into a folders under the solution, so if searching under solution, search with depth of 2
					var dirs = GetDirectories ( new ( basePath ) );
					foreach ( var subDir in dirs ) searchedDirs.AddRange ( GetDirectories ( subDir ) );
					searchedDirs.AddRange ( dirs );
				} else searchedDirs.AddRange ( GetDirectories ( new ( basePath ) ) );

				foreach ( var subdir in searchedDirs ) {
					potentialPath = Path.Combine ( subdir.FullName, filename );
					if ( Check ( potentialPath ) ) return true;
				}
			}
			potentialPath = null;
			return false;
		...
		if (CheckSubdirectories ( basePath, out var potentialSubdirPath ) ) return potentialSubdirPath;
		...
		if ( searchOptions.HasFlag ( SearchOptions.ProjectFolder ) || searchOptions.HasFlag ( SearchOptions.SolutionFolder ) ) {
			...
			if (searchOptions.HasFlag ( SearchOptions.SolutionFolder ) ) {
				// First is a project group, second is the actual solution
				if ( CheckPotentialPath ( potentialMainProj, out var potentialSolution, null ) ) return Return ();
				if ( CheckPotentialPath ( potentialSolution, out potentialSolution, null ) ) return Return ();

				if (Check(potentialSolution.FullName)) return potentialSolution.FullName;
				if (CheckSubdirectories ( potentialSolution.FullName, out var potentialSubdirPath2 ) ) return potentialSubdirPath2;
			}
		}
