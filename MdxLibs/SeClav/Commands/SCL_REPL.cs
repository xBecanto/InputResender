...
public class SCL_REPL : DCommand_CoreBase {
	...
	public SCL_REPL ( CoreBase owner, string parentDsc = null ) : base ( owner, parentDsc, CommandNames, InterCommands ) {
		parser = null;
	}
	...
	protected override CommandResult ExecIner ( CmdContext context ) {
		...
		case "reload": {
			var sclRunner = GetActiveCore<CoreBase> ()?.Fetch<SeClavRunnerCommand> ();
			if ( sclRunner == null )
