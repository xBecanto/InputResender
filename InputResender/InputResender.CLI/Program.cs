...
public static class Program {
	public static bool StartMain ( string[] args, DCommandLoader_IRCore TLLoader, CliWrapper cliWrapper ) {
		...
		if ( cliWrapper.CmdProc.Owner is not DInputResenderCore core )
			throw new NotImplementedException ( $"The core type '{cliWrapper.CmdProc.Owner.GetType ()}' is not a supported by the InputResender project." );
		...
		cliWrapper.CmdProc.AddCommand ( new BasicCommands ( core, cliWrapper.Console.WriteLine
				, cliWrapper.Console.Clear, () => { } /* Cleanup is done after main loop */
			), CommandProcessor.AddCmdBehavior.Skip
		);
		cliWrapper.CmdProc.AddCommand ( new FactoryCommandsLoader ( core ), CommandProcessor.AddCmdBehavior.Skip );
		cliWrapper.CmdProc.AddCommand ( new InputCommandsLoader ( core ), CommandProcessor.AddCmdBehavior.Skip );
		if ( TLLoader != null ) cliWrapper.CmdProc.AddCommand ( TLLoader, CommandProcessor.AddCmdBehavior.Skip );
	...
	public static void Main ( string[] args, DInputResenderCore initialCore, DCommandLoader_IRCore TLLoader, ConsoleManager console ) {
