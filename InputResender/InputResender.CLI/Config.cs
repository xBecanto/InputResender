...
public class Config : ComponentBase_IRCore {
	...
	public Config ( string initPath, PasswordHolder password, DInputResenderCore owner, string basePath = null ) : base ( owner ) {
	...
	private static void SetHomepathVar (DInputResenderCore Owner, string home) {
		if ( !Directory.Exists ( home ) ) throw new ArgumentException ( $"Home path '{home}' does not exist." );
		Owner.Fetch<CommandProcessor> ()
			.SetVar ( MdxLibs.Definitions.Commands.FileManagerCommand.HOME_PATH_VAR_NAME, home );
	}
