...
public class VScriptedInputProcessor : DInputProcessor {
	...
	public bool ExecSafeMode {
		...
		set {
			_execSafeMode = value;
			if ( Debugger != null ) { Debugger.LoggingRate = _execSafeMode ? 1 : -1; }
		}
	}
	...
	public override bool ProcessInput ( DataHolder[] inputCombination ) {
		...
		try {
			...
			ScriptRuntime.SetExternVar<BasicValueIntDef> ( "SettingChanged"
				, intDefinition => new BasicValueInt ( intDefinition, 0 ), Owner
			);
			WasFired = false;
			lock (ScriptRuntime) {
		...
		catch ( Exception ex ) { Owner.PushDelayedError ( "Error during execution of input processing script!", ex ); }
	...
	public class SCL_StatusType : SeClav.IDataType {
		...
		public SCL_StatusType (
			VScriptedInputProcessor comp, DataHolder[] inputCombination, SCLRuntimeHolder SCLruntime
		) {
			Processor = comp;
