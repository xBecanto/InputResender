...
public class InputSimulatorCommand : DCommand_IRCore {
	...
	public InputSimulatorCommand ( DInputResenderCore owner, string parentDsc = null )
...
public static HMouseEventDataHolder MouseMove ( ComponentBase owner, int X, int Y, int deviceID = 1 ) {
	...
	protected override CommandResult ExecIner ( CmdContext context ) {
		...
		if ( TryPrintHelp ( context.Args, context.ArgID + 1, () => context.SubAction switch {
			...
			"recapture" => "sim recapture [True/False]: Toggle whether simulated input can be captured by hooks. This is a global setting that affects all simulations, including those from other commands or generated by components.",
			"modmap" => $"sim modmap [<Modifier> <KeyCode>]: View or set the keycode mapping for a modifier. Without arguments, shows all mappings.\n\tModifier: {{{string.Join ( "|", Enum.GetNames<InputData.Modifier> () )}}}\n\tKeyCode: {{{string.Join ( "|", Enum.GetNames<KeyCode> () )}}} or 'None' to clear",
			_ => null
		...
		case "recapture": {
			string val = context.Args.String ( context.ArgID + 1, "Recapture value", 1, true );
			switch ( val.ToLower() ) {
				case "t": case "true": sim.AllowRecapture = true; break;
				case "f": case "false": sim.AllowRecapture = false; break;
				default: return new CommandResult ( $"Invalid value '{val}' for recapture. Use True(T) or False(F)." );
			}

			return new CommandResult ( $"Set recapture to {sim.AllowRecapture}." );
		}
		case "modmap": {
			// If no arguments, show all mappings
			if ( context.ArgID + 1 >= context.Args.ArgC ) {
				var mappings = sim.GetAllModifierMappings ();
				if ( mappings.Count == 0 ) {
					return new CommandResult ( "No modifier mappings configured." );
				}

				var lines = new List<string> { "Modifier mappings:" };
			...
			// Parse modifier and keycode arguments
			var modifier = context.Args.EnumC<InputData.Modifier> ( context.ArgID + 1, "Modifier", true );
			string keyCodeStr = context.Args.String ( context.ArgID + 2, "KeyCode", 1, true );

			// Check if clearing the mapping
			if ( keyCodeStr.Equals ( "None", StringComparison.OrdinalIgnoreCase ) ) {
				sim.SetModifierMapping ( modifier, KeyCode.None );
				return new CommandResult ( $"Cleared mapping for modifier {modifier}." );
			}

			// Parse and set the keycode
			var keyCode = context.Args.EnumC<KeyCode> ( context.ArgID + 2, "KeyCode", true );
			sim.SetModifierMapping ( modifier, keyCode );
			return new CommandResult ( $"Set modifier {modifier} to keycode {keyCode}." );
		}
		default: return new CommandResult ( $"Invalid action '{context.SubAction}'." );
		}
	}

	int Simulate ( CmdContext context, int offset, DInputSimulator sim, bool keyDown, bool keyUp, HHookInfo hInfo) {
	...
	public static List<KeyCode> GetKeysFromArgs (CmdContext context, int offset) {
