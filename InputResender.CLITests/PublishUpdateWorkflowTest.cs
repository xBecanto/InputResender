...
using Components.InterfaceTests;   // MockFileService, MockStreamWriter
...
public class PublishUpdateWorkflowTest {
	...
	private readonly CommandProcessor<DMainAppCore> _cmdProc;

	public PublishUpdateWorkflowTest ( ITestOutputHelper output ) {
		_output = output;

		var core = new DMainAppCoreFactory ().CreateVMainAppCore ( DMainAppCore.CompSelect.FileManager );
		_fm    = (VFileManager)core.Fetch<DFileManager> ();
		_mockFS = new MockFileService ();
		_fm.FileService = _mockFS;

		_cmdProc = new CommandProcessor<DMainAppCore> ( core, output.WriteLine );
		_cmdProc.AddCommand ( new FileManagerCommand ( core ) );
		_cmdProc.SetVar ( CoreManagerCommand<DMainAppCore>.ActiveCoreVarName, core );
	...
	public void UpdateCommand_DownloadsAndVerifiesZip_WithMockHttpAndMockFileSystem () {
		...
		// ── UPDATE COMMAND ────────────────────────────────────────────────────────
		var updateCore   = new DMainAppCoreFactory ().CreateVMainAppCore ( DMainAppCore.CompSelect.FileManager );
		...
		var updateCmdProc = new CommandProcessor<DMainAppCore> ( updateCore, _output.WriteLine );
		updateCmdProc.AddCommand ( updateCmd );
		updateCmdProc.SetVar ( CoreManagerCommand<DMainAppCore>.ActiveCoreVarName, updateCore );
	...
	private static string ExtractHashB64 ( string message ) {
		...
		string line = message.Split ( '\n' )
			.FirstOrDefault ( l => l.TrimStart ().StartsWith ( Marker ) );
		line.Should ().NotBeNull ( "command output must contain a 'Hash (base64):' line" );
...
sealed class TestableUpdateCommand : UpdateCommand {
	...
	public TestableUpdateCommand (
		DMainAppCore owner, HttpClient httpClient,
