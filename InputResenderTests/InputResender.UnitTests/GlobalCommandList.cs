...
internal class GlobalCommandList {
	...
	public GlobalCommandList () {
		...
		foreach ( Type type in allCmdTypes ) {
			if ( type.IsAssignableTo ( typeof ( DCommandLoader_CoreBase ) ) ) {
				ProcessLoader ( type );
			} else {
				ProcessCommand ( type );
			}
	...
	private void ProcessLoader ( Type loader ) {
		...
		try {
			var newCmdDict = GetField<KeyValuePair<Type, Func<DInputResenderCore, DCommand_CoreBase>>> ( loader, "NewCommandList", "Loader" );
			newCmdList = newCmdDict.Select ( kvp => kvp.Key ).ToList ();
		} catch ( Exception e1 ) {
			try {
				var newCmdDict = GetMethodListSimple<KeyValuePair<Type, Func<DInputResenderCore, DCommand_CoreBase>>> ( loader, "NewCommandList", "Loader" );
				newCmdList = newCmdDict.Select ( kvp => kvp.Key ).ToList ();
			} catch ( Exception e2 ) {
		...
		try {
			var newSubCmdList = GetField<KeyValuePair<Type, (string, Func<DCommand_CoreBase, DCommand_CoreBase>)>> ( loader, "NewSubCommandList", "Loader" );
	...
	private void FinalizeLoaders ( Dictionary<Type, Type> adHocSubs = null ) {
		...
		foreach ( var loader in loaderKeys ) {
			...
			foreach ( Type cmdT in origTypes ) {
				if ( cmdT.IsAssignableTo ( typeof ( DCommandLoader_CoreBase ) ) ) continue;
	...
	private void ProcessCommand ( Type cmdType ) {
		...
		foreach ( (string cmd, Type nextCmd) in interCmds ) {
			if ( nextCmd != null ) {
				if ( !nextCmd.IsAssignableTo ( typeof ( DCommand_CoreBase ) ) ) throw new CommandDefinitionException ( cmdType, $"Command '{cmdType.Name}' has invalid inter command type '{nextCmd.Name}'. It must be derived from 'DCommand_CoreBase'.", nextCmd );
